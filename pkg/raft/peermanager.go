...
type IPeerManager interface {
	...
	GoWaitAllPeers(action func(*Peer) interface{}) <-chan interface{}
...
// GoWaitAllPeers runs an action against all peers and wait for response
...
func (mgr *PeerManager) GoWaitAllPeers(action func(*Peer) interface{}) <-chan interface{} {
	peers := mgr.GetPeers()
	count := len(peers)
	replies := make(chan interface{}, count)
	defer close(replies)

	goaction := func(peer *Peer, wg *sync.WaitGroup) {
		go func() {
			ret := action(peer)
			if ret != nil {
				replies <- ret
